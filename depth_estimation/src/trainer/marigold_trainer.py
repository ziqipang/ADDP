...
class MarigoldTrainer:
    def __init__(
        ...
        # contribution-based sampling
        sampling_weights = torch.tensor([1., 1., 3., 7., 13., 14., 8., 4., 22., 33.])
        sampling_weights = sampling_weights.repeat_interleave(self.scheduler_timesteps // 10)
        sampling_weights = sampling_weights.contiguous()
        self.sampling_weights = sampling_weights / sampling_weights.sum()
    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                ...
                timesteps = torch.multinomial(self.sampling_weights, batch_size, replacement=True).long()
                timesteps = timesteps.to(device)

                depth_aug_for_latent = (depth_gt_for_latent + 1.) / 2.
                depth_aug_for_latent = depth_aug_for_latent.clamp(0., 1.)
                depth_aug_for_latent = depth_aug_for_latent.cpu().numpy()

                depth_aug_for_latent_list = []
                for batch_idx in range(batch_size):
                    intensity = self.intensity * ((timesteps[batch_idx]).item() + 1) / self.scheduler_timesteps
                    intensity = intensity * random.random()
                    depth_aug = depth_aug_for_latent[batch_idx].squeeze()
                    depth_aug = cv2.GaussianBlur(depth_aug, (31, 31), float(intensity))
                    depth_aug = torch.tensor(depth_aug).unsqueeze(0)
                    depth_aug_for_latent_list.append(depth_aug)

                depth_aug_for_latent = torch.stack(depth_aug_for_latent_list, dim=0).to(device)
                depth_aug_for_latent = depth_aug_for_latent * 2. - 1.
                ...
                with torch.no_grad():
                    ...
                    aug_depth_latent = self.encode_depth(
                        depth_aug_for_latent
                    )  # [B, 4, h, w]
                ...
                noisy_latents = self.training_noise_scheduler.add_noise(
                    aug_depth_latent, noise, timesteps
                )  # [B, 4, h, w]
