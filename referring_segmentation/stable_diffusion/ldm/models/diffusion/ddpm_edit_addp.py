...
class LatentDiffusion(DDPM):
    ...
    def shared_step(self, batch, **kwargs):
        ...
        encoder_posterior = self.encode_first_stage(batch["edited_aug"].to(self.device))
        x_aug = self.get_first_stage_encoding(encoder_posterior).detach()
        t = batch["t"].to(self.device).long()
        loss = self(x, x_aug, c, t)
        return loss

    def forward(self, x, x_aug, c, t, *args, **kwargs):
        ...
        return self.p_losses(x, x_aug, c, t, *args, **kwargs)
    ...
    def p_losses(self, x_start, x_aug, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_aug))
        x_noisy = self.q_sample(x_start=x_aug, t=t, noise=noise)
